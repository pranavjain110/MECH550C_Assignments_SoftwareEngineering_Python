...
class ComplexNumbers:
    ...
    #def __del__(self): 
        #print('Destructor called, ComplexNumber object deleted.') 
...
class ComplexMultiplication:
    ...
    def __del__(self): 
        print('|Destructor called, ComplexMultiplication object deleted|') 
        if bool(__debug__):
...
def multiplyMatrices():
    def listOfTuples(row,col):
        ...
        for i in range(row):
            for j in range(col):
                re = floatInput("Enter the Real part of element({},{}): ".format(i+1,j+1))
                im = floatInput("Enter the Complex part({},{}): ".format(i+1,j+1))
